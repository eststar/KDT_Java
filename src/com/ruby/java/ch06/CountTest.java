...
class Count { // 232페이지
	static int totalCount;// 메소드 영역에 저장. 정적변수. 클래스필드
	int count = 0;// heap 영역에 저장.인스턴스필드

	public static void print1() { // 클래스 메소드. 정적메서드
...
public class CountTest { // file 이름과 같아야 한다
	...
	public static void main(String[] args) {
		Count.print1(); //외부 클래스의 메서드 호출
//		print1(); // 외부 클래스의 메서드기 때문에 그냥 사용 불가
		
		Count.totalCount++;// 클래스 필드의 사용 235페이지 (2)클래스 필드 사용. static 변수. 객체 생성 없이 가능
//		Count.count++;//오류 발생. 클래스 멤버 변수. 인스턴스 필드이기 때문에 객체 생성후 접근해야함

		showStatic(); //클래스 내에 있으므로 "클래스명." 생략 가능.
...
//		show();
		...
		c1.count++;
		//c1.totalCount++;// 클래스 필드를 접근하는 바람직한 방법 아님. "클래스명." 생략하는게 좋음
