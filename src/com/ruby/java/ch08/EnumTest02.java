...
//		Mandarin m;//생성자를 호출하지 않는다
		...
		//*
		/*
		 * main() 메서드에서 enum 상수에 최초로 접근하는 시점에 생성자들이 모두 호출. 객체들을 모두 만듬
		 * Mandarin.values() 또는 Mandarin.한라봉 등이 처음 호출되는 그 순간에 
		 * 모든 enum 상수들의 생성자가 한꺼번에 호출.
		 * .values()등의 enum 메서드들을 호출하거나, enum 객체 하나를 호출하면 enum에 포함된 모든 상수 객체들의 생성자가 호출되어 모두 생성함. 
		 * 이후 사용하더라도 이미 생성되었기 때문에 중복으로 생성 하지 않음
		 */
//		Mandarin ma = Mandarin.valueOf("한라봉");
