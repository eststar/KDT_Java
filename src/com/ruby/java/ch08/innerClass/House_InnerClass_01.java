...
class House {
	...
	private Room[] rooms; //Room 객체의 필드 만큼의 크기 x 배열 크기만큼 메모리 할당
	private int numRooms; //생성자에서 배열 크기
	private int count; //방 2개 이면 count = 2
...
public class House_InnerClass_01 {
...
//		House.Room myRoom = new House.Room("내방");//오류
		...
		// 내부 클래스를 new 할경우 외부 클래스의 객체가 생성되어야 그것의 내부 클래스의 객체생성 new 생성자() 가능?
		myRoom.show();
	}
}